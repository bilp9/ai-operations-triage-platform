...
CATEGORY_SPECS = {
    "safety_incident": {
        ...
        "templates": (
            ("high", "A caller reported a collision at site {site}. One person is hurt and incident {reference} needs review."),
            ("high", "The driver says the area is unsafe after a threat near location {site}. Ticket {reference} was opened."),
            ("critical", "There is a fire at location {site} and people may be in immediate danger. Incident {reference}."),
            ("critical", "A weapon was reported near site {site}. This is an emergency linked to case {reference}."),
            ("high", "An associate slipped and may have a sprain at site {site}. Please escalate report {reference}."),
            ("critical", "Heavy smoke is spreading through the loading area at {site}; evacuation is underway for report {reference}."),
        ),
    },
    "property_damage": {
        ...
        "templates": (
            ...
            ("high", "The delivery driver damaged the garage at location {site}. Claim case {reference}."),
    ...
    "general_support": {
        ...
        "templates": (
            ...
            ("low", "Where can I find the current service hours for location {site}?"),
...
def build_rows() -> list[dict[str, str]]:
    ...
    for category, spec in CATEGORY_SPECS.items():
        for index in range(50):
            ...
            site = f"AZ{10 + serial % 17}"
            email = f"demo.user{serial:03d}@example.com"
            transcript = template.format(reference=reference, site=site, email=email)
            if index % 7 == 0:
